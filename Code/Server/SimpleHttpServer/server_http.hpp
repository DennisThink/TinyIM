...
namespace SimpleWeb {
  ...
  public:
    ...
    virtual void start() {
      ...
      if(!acceptor)
        acceptor = std::unique_ptr<asio::ip::tcp::acceptor>(new asio::ip::tcp::acceptor(m_IoService));
  ...
  public:
    Server(asio::io_service& ioService) noexcept : ServerBase<HTTP>::ServerBase(ioService,80) {}

  protected:
    void accept() override {
      auto connection = create_connection(m_IoService);
